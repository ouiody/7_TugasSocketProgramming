...
def send_messages_to_server():
    ...
    if message:
        try:
            # Kirim pesan asli (tanpa prefix username)
...
def receive_messages_from_server():
    while True:
        try:
            ...
            chat_display.config(state='disabled')
            chat_display.yview(tk.END)  # Scroll otomatis ke pesan terbaru
        except socket.timeout:
            # Jika terjadi timeout, biarkan loop tetap berjalan
            continue
        except OSError:
            # Jika socket ditutup, keluar dari loop
            break
        except Exception as e:
            messagebox.showerror("Error", f"Gagal menerima pesan: {e}")
            break
